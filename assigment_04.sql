...
SELECT *
FROM account
JOIN postion USING (position_id)
...
SELECT code.*
FROM exam 
JOIN duration USING (duration_id)
GROUP BY duration_id 
ORDER BY count(duration_id) DESC;
-- Question 6: Thông kê mỗi category Question được sử dụng trong bao nhiêu question
SELECT category_question.*, COUNT(question_id) AS question_count
From  question 
 RIGHT JOIN category quesion USING (category_id)
Group BY category_id;
-- Question 7: Thông kê mỗi Question được sử dụng trong bao nhiêu Exam
SELECT question_id.*, COUNT(exam_id) AS exam_count 
FROM exam_question 
...
FROM account 
JOIN department USING (department_id)
WHERE COUNTIFS  = "dev,test,scrum, master,PM"
GROUP BY department_id;
...
WHERE COUNT(question_id)

-- question, loại câu hỏi, ai là người tạo ra câu hỏi, câu trả lời là gì, …
-- Question 13: Lấy ra số lượng câu hỏi của mỗi loại tự luận hay trắc nghiệm
SELECT COUNT.* 
FROM questions
WHERE question_type = 'Tự luận';
UNION 
SELECT COUNT.*
FROM questions
WHERE question_type = 'Trắc Nghiệm';

-- Question 14:Lấy ra group không có account nào
SELECT `group`.*
    FROM `group` 
    LEFT JOIN group_account  USING (group_id)
    GROUP BY account_id IS NULL;
...
SELECT *
FROM `group`
INNER JOIN menbers USING (group_id)
WHERE group_id > 5;
SELECT *
FROM `group`
INNER JOIN menbers USING (group_id)
